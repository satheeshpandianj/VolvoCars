...
Date : 03-03-2021
Script name: WorkloadMixScript.js
...
export function getUsers()
{
	...
	var url = 'https://reqres.in/api/users?page=2'; // application url
 	let getRes = http.get(url,{
 								tags: 
 								{ 
 									my_custom_tag: 'getUsers' 
 								},
 							  }
 						 );
	...
	let getResultCheck = check(getRes, {
											"status is HTTP 200": g => g.status === 200
										}
							  ); 
...
export function postUsers()
{
	...
	var url = "https://reqres.in/api/users"; // application url
	...
	let postResultCheck = check(postRes, {
											"status is HTTP 201": p => p.status === 201
										 }
							   );
...
export function putUsers()
{
	...
	var putUrl = 'https://reqres.in/api/users/2'; // application url
	var payload = JSON.stringify({
  									'name':'PT' + __ITER + __VU,
  									'job':'executive'
  								 }
	...
	let putResultCheck = check(putRes, {
											"status is HTTP 200": pt => pt.status === 200 
										}
							  );
